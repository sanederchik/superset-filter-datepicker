...
const propTypes = {
  ...
  showDatePickerFilter: PropTypes.bool,
...
class FilterBox extends React.PureComponent {
  ...
  renderDatePickerFilter(filterConfig) {
  ...
  renderFilters() {
    ...
    //another option if showDatePicker
    if(this.props.showDatePickerFilter){
      return filtersFields.map(filterConfig => {
        const { label, key } = filterConfig;
        return (
          <div key={key} className="m-b-5 filter-container">
            {this.renderDatePickerFilter(filterConfig)}
          </div>
        );
      });
    }
